...
(function() {
  window.Table = (function() {
    ...
    Table.prototype.getColDest = function(colnum){
        var colToGet = colnum;
        var offsets = [];
        var skips = [];
        var cells = [];
    ...
    Table.prototype.moveColumn = function($src, $src_copy, $dest) {
      ...
      $elements_dest = this.getColDest(column_dest);
      ...
      for (i = k = 1, ref = $elements_src.length - 1; k <= ref; i = k += 1) {
        $src = $elements_src[i];
        if($src[0]){
          $src_copy = $src[1].clone();
          $dest = $elements_dest[i];
          if($dest[0] == 'tr'){
            $dest[1].prepend($src_copy);
          }
          else if ($dest[0] == 'td'){
            $src_copy.insertAfter($dest[1]);
          }
          this.$table.trigger('cellMoved', [$src_copy]);
          $src_copy.find('.item').each((function(_this) {
            return function(i, elem) {
              return $src[1].trigger('itemMoved', [$(elem)]);
            };
          })(this));
          results.push($src[1].remove());
        }
      }
      return results;
    };
