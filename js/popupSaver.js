// Partially generated by CoffeeScript 1.9.3 (TODO : Documentation expliquant le "coffescript au début, bati en jquery au dessus")
(function() {
  ...
  window.PopupSaver = (function(superClass) {
    ...
    PopupSaver.prototype.create = function(notions) {
      ...
      $structure_checkbox = this.createCheckbox('', 'save_structure', true);
      ...
      $label_structure = this.createLabel('save_tags', 'Tags').prepend(this.createCheckbox('', 'save_tags', true));
      $group.append($label_structure);
      $body.append($group);
      ...
      for (i = 0, len = notions.length; i < len; i++) {
        ...
        $input_notion = this.createCheckbox(notion.getName(), 'notion_' + notion.getName(), true);
        $td_notion = $('<td></td>').append($input_notion);
        
        $input_classes = this.createCheckbox(notion.getName(), 'classes_' + notion.getName(), true);
        $td_classes = $('<td></td>').append($input_classes);
        
        $input_instances = this.createCheckbox(notion.getName(), 'instances_' + notion.getName(), true).addClass('instance_checkbox');
        ...
        $input_instances.on({
          change: function() {
            ...
            if (is_checked === true) {
              ...
              // We must have the table structure if we want the instances
      ...
      $save_button = this.createInputFileSave();
      $save_button.on({
        click: function() {
          //initialisation of working directory and file name
          var fs = require('fs');
          var os = require('os');
        ...
        change: (function(_this) {
          return function() {
            _this.$popupsaver.trigger('saverSet', [this.value, to_save]);
            return _this.close();
          };
        })(this)
      });
      ...
      $hidden_fi = $('<div hidden></div>').append($save_button);
      
      this.$popupsaver = this.createPopup([this.createTitle('Save As')], [$body], [$menu, $hidden_fi], 'saver');
